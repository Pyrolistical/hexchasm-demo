...
function completedDates() {
  ...
  return new Set(raw === undefined ? [] : JSON.parse(raw));
}
...
class Game {
  ...
  revealTimeouts;
  revealHoldTimeout;
  constructor(puzzle, date, definitions, renderer) {
    this.date = date;
    this.found = [];
  ...
  frame() {
    ...
    if (this.won) {
      this.renderer.drawCleared(now - this.wonTime);
    }
...
async function main() {
  ...
  if (param !== undefined) {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(param) || Number.isNaN(parsed)) {
      ...
      showMessage(`Invalid date: ${param}`);
      return;
    }
    targetUtc = parsed;
  }
  ...
  if (targetUtc > todayUtc) {
    showMessage(`The puzzle for ${dateString(targetUtc)} isn't available yet.`);
    return;
  }
  const dayIndex = (targetUtc - START_UTC) / DAY_MS;
  if (dayIndex < 0) {
    showMessage(`Puzzles start on ${dateString(START_UTC)}.`);
    return;
  }
  if (dayIndex >= index.total) {
    showMessage(`No puzzle for ${dateString(targetUtc)}.`);
    return;
  }
  ...
  const game = new Game(puzzle, dateString(targetUtc), definitions, renderer);
  window.game = game;
  game.start();
}
main();
