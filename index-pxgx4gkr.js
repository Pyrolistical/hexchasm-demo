...
var assertTypeOf = (value, expectedTypeOf) => {
...
// src/core/hex.ts
var asCoordinateKey = ({ q, r, s }) => `${q} ${r} ${s}`;
var ringOf = ({ q, r, s }) => Math.max(Math.abs(q), Math.abs(r), Math.abs(s));
var hexNeighborOffsets = [
  { q: 1, r: -1, s: 0 },
  { q: 1, r: 0, s: -1 },
  { q: 0, r: 1, s: -1 },
  { q: -1, r: 1, s: 0 },
...
function* gridCoordsGenerator(size) {
...
var coordinateToPixel = ({ q, r }, gridScale) => ({
  x: 3 / 2 * q * gridScale,
  y: Math.sqrt(3) / 2 * (2 * r + q) * gridScale
});
var hexVerts = (gridScale) => [
  [-1 * gridScale, 0],
  [-0.5 * gridScale, -(Math.sqrt(3) / 2) * gridScale],
  [0.5 * gridScale, -(Math.sqrt(3) / 2) * gridScale],
  [1 * gridScale, 0],
  [0.5 * gridScale, Math.sqrt(3) / 2 * gridScale],
  [-0.5 * gridScale, Math.sqrt(3) / 2 * gridScale]
];
var hexGroupOutlineSegments = (group, gridScale) => {
  const groupSet = new Set(group.map(asCoordinateKey));
  const verts = hexVerts(gridScale);
  const segments = [];
  for (const coord of group) {
    const { x, y } = coordinateToPixel(coord, gridScale);
    for (let i2 = 0;i2 < 6; i2++) {
      const nb = edgeNeighbors[i2];
      const neighborKey = asCoordinateKey({
        q: coord.q + nb.q,
        r: coord.r + nb.r,
        s: coord.s + nb.s
      });
      if (!groupSet.has(neighborKey)) {
        const [x1, y1] = verts[i2];
        const [x2, y2] = verts[(i2 + 1) % 6];
        segments.push(`M ${x + x1} ${y + y1} L ${x + x2} ${y + y2}`);
      }
    }
  }
  return segments;
...
function* findMatches(state) {
  ...
  for (const startKey of Object.keys(state)) {
    if (checked2.has(startKey))
      continue;
    const targetValue = state[startKey]?.value;
    ...
    function* visit(key) {
      ...
      checked2.add(key);
      yield key;
      const { q, r, s } = state[key].coordinate;
      for (const offset of hexNeighborOffsets) {
        ...
        if (!checked2.has(neighborKey) && state[neighborKey]?.value === targetValue) {
          yield* visit(neighborKey);
        }
      }
    }
    const group = [...visit(startKey)];
    if (group.length >= 4)
      yield group;
  }
}
var pickWinner = (state, group, preferred) => {
  if (preferred && group.includes(preferred))
    return preferred;
  return group.reduce((best, key) => {
    const bc = state[best].coordinate;
    const kc = state[key].coordinate;
    if (kc.s !== bc.s)
      return kc.s < bc.s ? key : best;
    return kc.q < bc.q ? key : best;
  });
};
var settle = (initialState, clickedKey, size, random, heartsAtStart) => {
  const state = {};
  ...
  state[clickedKey] = {
    ...state[clickedKey],
    value: state[clickedKey].value + 1
  };
  const frames = [];
  let matchMade = false;
  let preferred = clickedKey;
  let hearts = heartsAtStart;
  while (true) {
    const matches = [...findMatches(state)];
    if (matches.length === 0)
      break;
    matchMade = true;
    const resolved = matches.map((group) => ({
      group,
      winnerKey: pickWinner(state, group, preferred)
    }));
    preferred = undefined;
    const groupsSum = resolved.reduce((acc, { group, winnerKey }) => acc + group.length * state[winnerKey].value, 0);
    const scoreDelta = Math.round(Math.pow(groupsSum, hearts));
    frames.push({
      type: "match",
      groups: resolved.map(({ group, winnerKey }) => ({
        nonWinners: group.filter((k) => k !== winnerKey).map((k) => ({ coord: state[k].coordinate, value: state[k].value })),
        winner: {
          coord: state[winnerKey].coordinate,
          value: state[winnerKey].value
        },
        newValue: state[winnerKey].value + 1,
        groupSum: group.length * state[winnerKey].value
      }))
    });
    let nextScore = {
      groupsSum,
    ...
    hearts += 1;
    for (const { group, winnerKey } of resolved) {
      const newValue = state[winnerKey].value + 1;
      for (const key of group) {
        if (key === winnerKey) {
          state[winnerKey] = {
            coordinate: state[winnerKey].coordinate,
            value: newValue
          };
        } else {
          delete state[key];
    ...
    while (true) {
      const occupied = new Set(Object.keys(state));
      const moves = [];
      for (const source of gridCoords(size).sort((a, b) => a.s - b.s)) {
        const sourceKey = asCoordinateKey(source);
        ...
        if (ringOf(dest) > size || occupied.has(asCoordinateKey(dest)))
          continue;
        occupied.delete(sourceKey);
        occupied.add(asCoordinateKey(dest));
        moves.push({ from: source, to: dest, value: state[sourceKey].value });
      }
      if (moves.length > 0) {
        frames.push({
          type: "gravity",
        ...
        nextScore = undefined;
        for (const { from: from2, to, value } of moves) {
          state[asCoordinateKey(to)] = { coordinate: to, value };
          delete state[asCoordinateKey(from2)];
        }
        continue;
      }
      const spawnCells = [];
      for (const coord of gridCoords(size)) {
        if (occupied.has(asCoordinateKey(coord)))
          continue;
        const spawnFrom = {
          q: coord.q,
          r: coord.r - 1,
        ...
        if (ringOf(spawnFrom) <= size)
          continue;
        const value = Math.floor(random() * 6) + 1;
        state[asCoordinateKey(coord)] = { coordinate: coord, value };
        spawnCells.push({ coord, spawnFrom, value });
      }
      if (spawnCells.length > 0) {
        frames.push({ type: "spawn", cells: spawnCells });
        continue;
      }
      break;
    }
  }
  return { frames, matchMade, finalState: state };
};
...
var valueToFill = (value) => `var(${cellFills[value % cellFills.length]})`;
var gridScale = 1000 / (size * 3 + 2);
{
  const width = 1000;
  ...
  document.body.insertAdjacentHTML("beforeend", `<main>
      ...
      >
        <g class="grid" transform="translate(${gridX}, ${gridY})">
        </g>
      </svg>
      <aside class="sidebar">
...
svg.onclick = (event) => event.preventDefault();
var grid = svg.querySelector(".grid");
assertInstanceOf(grid, SVGGElement);
var heartsEl = main.querySelector(".hearts");
assertInstanceOf(heartsEl, HTMLElement);
var gameOverEl = main.querySelector(".game-over");
assertInstanceOf(gameOverEl, HTMLElement);
var scoreEl = main.querySelector(".score");
assertInstanceOf(scoreEl, HTMLElement);
var MAX_HEARTS = 2;
var hearts = MAX_HEARTS;
var score = 0;
var renderHearts = () => {
  heartsEl.textContent = "♥".repeat(Math.min(MAX_HEARTS, hearts)) + "♡".repeat(MAX_HEARTS - Math.min(MAX_HEARTS, hearts));
};
var formatScore = (n) => {
  if (n < 1000)
    return String(n);
  const units = ["k", "M", "G", "T", "P", "E"];
  const i2 = Math.min(Math.floor(Math.log10(n) / 3) - 1, units.length - 1);
  const divisor = Math.pow(10, (i2 + 1) * 3);
  const scaled = n / divisor;
  const magnitude = scaled < 10 ? 2 : scaled < 100 ? 1 : 0;
  const rounded = parseFloat(scaled.toFixed(magnitude));
  if (rounded >= 1000 && i2 + 1 < units.length) {
    return (rounded / 1000).toFixed(2) + units[i2 + 1];
  }
  return rounded.toFixed(magnitude) + units[i2];
};
var renderScore = () => {
...
renderHearts();
renderScore();
var cellCoordinateKey = (cell) => cell.getAttribute("data-coordinate");
var isCell = (element) => element instanceof SVGGElement && element.classList.contains("cell");
var random = import_seedrandom.default("1234");
var state = {};
{
  for (const coordinate of gridCoordsGenerator(size)) {
    state[asCoordinateKey(coordinate)] = {
      coordinate,
      value: Math.floor(random() * 6) + 1
    };
  }
  grid.addEventListener("contextmenu", (event) => event.preventDefault());
  const cellHTML = (key, coord, value, pos) => `<g style="transform: translate(${pos.x}px, ${pos.y}px)">
      <g class="cell" data-coordinate="${key}">
        <path
          d="
            m ${-gridScale} 0
            l ${0.5 * gridScale} ${-(Math.sqrt(3) / 2) * gridScale}
            l ${gridScale} 0
            l ${0.5 * gridScale} ${Math.sqrt(3) / 2 * gridScale}
            l ${-0.5 * gridScale} ${Math.sqrt(3) / 2 * gridScale}
            l ${-gridScale} 0
        ...
        <text
          ...
          fill="white"
          stroke="black"
          stroke-width="${0.2 * gridScale}"
          font-size="${gridScale * 0.9}"
          font-weight="bolder"
          text-anchor="middle"
          dominant-baseline="middle"
          transform="translate(0, ${0.09 * gridScale})"
        >
          ${value}
        </text>
      </g>
    </g>`;
  for (const { coordinate, value } of Object.values(state)) {
    grid.insertAdjacentHTML("beforeend", cellHTML(asCoordinateKey(coordinate), coordinate, value, coordinateToPixel(coordinate, gridScale)));
  }
  const animateTransition = (wrapper, fromX, fromY, toX, toY, dur, extraProps) => {
    wrapper.style.transition = "none";
    wrapper.style.transform = `translate(${fromX}px, ${fromY}px)`;
    if (extraProps)
      wrapper.style.opacity = extraProps.fromOpacity;
    wrapper.getBoundingClientRect();
    const props = extraProps ? `transform ${dur}ms linear, opacity ${dur}ms linear` : `transform ${dur}ms linear`;
    wrapper.style.transition = props;
    wrapper.style.transform = `translate(${toX}px, ${toY}px)`;
    if (extraProps)
      wrapper.style.opacity = extraProps.toOpacity;
    return new Promise((resolve) => wrapper.addEventListener("transitionend", () => resolve(), {
      once: true
    }));
  };
  const animateGravityMove = async (from2, to) => {
    const fromKey = asCoordinateKey(from2);
    const toKey = asCoordinateKey(to);
    const cellEl = grid.querySelector(`.cell[data-coordinate="${fromKey}"]`);
    if (!cellEl)
      return;
    assertInstanceOf(cellEl.parentElement, SVGGElement);
    cellEl.setAttribute("data-coordinate", toKey);
    const { x: fromX, y: fromY } = coordinateToPixel(from2, gridScale);
    const { x: toX, y: toY } = coordinateToPixel(to, gridScale);
    await animateTransition(cellEl.parentElement, fromX, fromY, toX, toY, 160);
  };
  const animateSpawn = async (coord, value, spawnFrom) => {
    const key = asCoordinateKey(coord);
    const spawnPos = coordinateToPixel(spawnFrom, gridScale);
    const destPos = coordinateToPixel(coord, gridScale);
    grid.insertAdjacentHTML("beforeend", cellHTML(key, coord, value, spawnPos));
    const cellEl = grid.querySelector(`.cell[data-coordinate="${key}"]`);
    assertInstanceOf(cellEl.parentElement, SVGGElement);
    await animateTransition(cellEl.parentElement, spawnPos.x, spawnPos.y, destPos.x, destPos.y, 160);
  };
  const animateTowardWinner = async (nonWinner, winnerCoord) => {
    const key = asCoordinateKey(nonWinner.coord);
    const wrapper = grid.querySelector(`.cell[data-coordinate="${key}"]`)?.parentElement;
    if (!wrapper)
      return;
    assertInstanceOf(wrapper, SVGGElement);
    const { x: fx, y: fy } = coordinateToPixel(nonWinner.coord, gridScale);
    const { x: tx, y: ty } = coordinateToPixel(winnerCoord, gridScale);
    await animateTransition(wrapper, fx, fy, tx, ty, 400, {
      fromOpacity: "1",
      toOpacity: "0"
    });
  };
  const flashGroupOutline = async (coords) => {
    const segments = hexGroupOutlineSegments(coords, gridScale);
    if (segments.length === 0)
      return;
    grid.insertAdjacentHTML("beforeend", `<path
        class="group-outline"
        d="${segments.join(" ")}"
        fill="none"
        stroke="var(--foreground)"
        stroke-width="${0.08 * gridScale}"
        stroke-linecap="round"
      />`);
    const outline = grid.lastElementChild;
    await new Promise((resolve) => outline.addEventListener("animationend", () => {
      outline.remove();
      resolve();
    }, { once: true }));
  };
  const animateMatch = async (nonWinners, winner, newValue) => {
    await flashGroupOutline([...nonWinners.map((c) => c.coord), winner.coord]);
    await Promise.all(nonWinners.map((nw) => animateTowardWinner(nw, winner.coord)));
    for (const { coord } of nonWinners) {
      grid.querySelector(`.cell[data-coordinate="${asCoordinateKey(coord)}"]`)?.parentElement?.remove();
    }
    const winnerEl = grid.querySelector(`.cell[data-coordinate="${asCoordinateKey(winner.coord)}"]`);
    if (winnerEl) {
      winnerEl.querySelector("path").setAttribute("fill", valueToFill(newValue));
      winnerEl.querySelector(".label").textContent = String(newValue);
    }
  };
  const makeSvgLabel = (x, y, text, fontSize = gridScale * 0.7) => {
  ...
  const animateMatchScore = async (frame) => {
    const groupLabels = frame.groups.map(({ winner, groupSum }) => {
      const { x, y } = coordinateToPixel(winner.coord, gridScale);
      return makeSvgLabel(x, y, String(groupSum));
    });
    await wait(700);
    const flyDur = 300;
    await Promise.all(groupLabels.map((el) => new Promise((resolve) => {
      el.style.transition = `transform ${flyDur}ms cubic-bezier(0.4, 0, 0.2, 1), opacity ${flyDur}ms ease-in`;
  ...
  const animateScoreDelta = async (groupsSum, hearts2, scoreDelta) => {
    const heartsLabel = makeSvgLabel(0, 0, `${formatScore(groupsSum)}^${"♥".repeat(hearts2)}`, gridScale * 0.6);
    await wait(700);
    heartsLabel.textContent = `+${formatScore(scoreDelta)}`;
    heartsLabel.style.transition = "none";
    heartsLabel.getBoundingClientRect();
    await wait(700);
    score += scoreDelta;
    renderScore();
    const rightEdge = size * gridScale * 1.5;
    heartsLabel.style.transition = "transform 400ms ease-in, opacity 400ms ease-in";
    heartsLabel.style.transform = `translate(${rightEdge}px, 0px)`;
    heartsLabel.style.opacity = "0";
    await new Promise((resolve) => heartsLabel.addEventListener("transitionend", () => resolve(), {
      once: true
    }));
    heartsLabel.remove();
  };
  const animate = async (frames) => {
    for (const frame of frames) {
      if (frame.type === "match") {
        await Promise.all([
          ...frame.groups.map(({ nonWinners, winner, newValue }) => animateMatch(nonWinners, winner, newValue)),
          animateMatchScore(frame)
        ]);
      } else if (frame.type === "gravity") {
        if (frame.nextScore !== undefined) {
          animateScoreDelta(frame.nextScore.groupsSum, frame.nextScore.hearts, frame.nextScore.scoreDelta);
        }
        await Promise.all(frame.moves.map(({ from: from2, to }) => animateGravityMove(from2, to)));
      } else if (frame.type === "spawn") {
        await Promise.all(frame.cells.map(({ coord, spawnFrom, value }) => animateSpawn(coord, value, spawnFrom)));
      }
    }
  };
  let settling = false;
  ...
  grid.addEventListener("pointerdown", (event) => {
    lastButtonDown = event.buttons;
  });
  grid.addEventListener("pointerup", async (event) => {
    if (settling || hearts === 0 || ![LEFT_CLICK, RIGHT_CLICK].includes(lastButtonDown)) {
      return;
    }
    for (const hit of event.composedPath()) {
      if (isCell(hit)) {
        const coordinateKey = cellCoordinateKey(hit);
        const { frames, matchMade, finalState } = settle(state, coordinateKey, size, random, hearts);
        if (matchMade) {
          hearts = MAX_HEARTS;
        } else {
          hearts -= 1;
        ...
        renderHearts();
        const clickedValue = state[coordinateKey].value + 1;
        hit.querySelector("path").setAttribute("fill", valueToFill(clickedValue));
        hit.querySelector(".label").textContent = String(clickedValue);
        settling = true;
